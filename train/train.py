...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default="jsonl",
        choices=["jsonl", "canvas"],
        help="jsonl: HuggingFace JSON dataset; canvas: CSV-driven CanvasSceneDataset.",
    )
    parser.add_argument(
        "--csv_path",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="Dataloader validation: batch size for one random validation batch (capped by dataset size).",
    )
    parser.add_argument(
    ...
    if args.dataset_type == "canvas":
        ...
        if not args.canvas_random_target_resolution:
            _train_lz(
                ...
                f"(target+canvas, resize={args.canvas_unified_resize}); "
                f"LoRA cond {args.unified_train_width}x{args.unified_train_height}",
            )
        else:
