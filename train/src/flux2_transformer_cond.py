...
class Flux2TransformerBlockCond(nn.Module):
    ...
    def forward(
        ...
        temb_mod_params_img_cond: Optional[Tuple[Tuple[torch.Tensor, torch.Tensor, torch.Tensor], ...]],
...
class Flux2SingleTransformerBlockCond(nn.Module):
    ...
    def forward(
        ...
        temb_mod_params_cond: Optional[Tuple[torch.Tensor, torch.Tensor, torch.Tensor]],
        image_rotary_emb: Optional[Tuple[torch.Tensor, torch.Tensor]] = None,
        joint_attention_kwargs: Optional[Dict[str, Any]] = None,
        ...
        if use_cond:
            c_shift, c_scale, c_gate = temb_mod_params_cond
...
class Flux2Transformer2DModelCond(
    ...
    def forward(
        ...
        single_stream_mod_cond = self.single_stream_modulation(cond_temb)[0] if use_cond else None
