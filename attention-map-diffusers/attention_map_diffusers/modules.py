...
# Flux2AttnProcessor - for Flux2Transformer2DModel (black-forest-labs/FLUX.2-dev)
def flux2_attn_call(
    self,
    attn: "Flux2Attention",
    hidden_states: torch.Tensor,
    ...
    """Custom __call__ for Flux2AttnProcessor to capture cross-attention (image->text)."""
    ...
    if hasattr(self, "store_attn_map") and encoder_hidden_states is not None:
        ...
        # attention_probs: (B, heads, text+img, text+img). Image->text: [:, :, text_len:, :text_len]
        attention_probs = attention_probs[:, :, text_length:, :text_length].cpu()
        ...
        # Flux2 passes timestep/1000; preserve for attn_maps key
        self.timestep = float(timestep.item()) if timestep is not None and timestep.numel() == 1 else (float(timestep[0].cpu().item()) if timestep is not None else 0)
    else:
        hidden_states = F.scaled_dot_product_attention(
            query, key, value, attn_mask=attention_mask, dropout_p=0.0, is_causal=False
        )
